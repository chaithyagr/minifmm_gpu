...
void init_with_known_data(t_fmm_params* params, TYPE* weights, TYPE* points, long num_points, int num_terms, int ncrit, float theta, int num_samples)
{
    ...
    params->reorder = (int*)malloc(sizeof(int)*params->num_points);
    params->reorder_ordered = (int*)malloc(sizeof(int)*params->num_points);
    ...
    for (size_t i = 0; i < params->num_points*3; ++i) params->points[i] = points[i];
    for (size_t i = 0; i < params->num_points*3; ++i) params->acc[i] = 0.0;
    for (size_t i = 0; i < params->num_points; ++i)
    {
        params->pot[i] = 0.0;
        params->reorder[i] = i;
        params->reorder_ordered[i] = i;
    }
